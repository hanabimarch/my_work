...
def make_phaI_with_events(bn,ni,topdir,savedir,slice_start,slice_stop,time_start = -100,time_stop = 300):
	...
	for i in ch_n:
		...
		total_count[i] = len(s_ch_index)+len(b_ch_index)#总光子数
		bkg_count[i] = len(b_ch_index)#背景光子数
		bkg_uncertainty[i] = np.sqrt(bkg_count[i] / exposure)
		total_uncertainty[i] = np.sqrt(total_count[i] / exposure)
	write_phaI(total_count, bn, ni, slice_start, slice_stop, savedir + 'A_' + bn + '_' + ni + '.pha', exposure)
	write_phaI(bkg_count, bn, ni, slice_start, slice_stop, savedir + 'A_' + bn + '_' + ni + '.bkg', exposure)
	x = np.sqrt(emin * emax)
...
def make_phaI(bn,ni,topdir,savedir,slice_start,slice_stop,binsize = 1,time_start = -100,time_stop=300):
	...
	'''
	if (type(slice_start).__name__ == 'int' or type(slice_start).__name__ == 'float'):
...
def write_phaI(spectrum_counts,bnname,detector,t1,t2,outfile,n,exposure):
	'''
	...
	if outfile[-3:]=='pha':
		header.append(('backfile', 'A_'+bnname+'_'+detector+'_'+n+'.bkg', 'Name of corresponding background file (if any)'))#这里有背景文件的名字
